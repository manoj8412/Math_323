	...
	\begin{center}
		\begin{tikzpicture}
			...
			\foreach \x in {0, ..., 10}
				\draw (\x, -0.1) -- (\x, 0.1);
			\draw 
				(0,0) node[below=-1.6mm]{$\bullet$} node[above]{0.1} node[below]{0}
				(5,0) node[below=-1.6mm]{$\bullet$} node[above]{0.6} node[below]{1}
				(10,0) node[below=-1.6mm]{$\bullet$} node[above]{0.3} node[below]{2}
			;
		\end{tikzpicture}
	\end{center}
	\begin{figure}[h]
	...
	\hfill
		\begin{subfigure}{0.49\textwidth}
			\begin{center}
				\begin{tikzpicture}
					% Axes
					\draw [->, >=stealth, auto] (0,0) to (2.5, 0) node[right]{$x$} ;
					\draw [->, >=stealth, auto] (0,0) to (0, 3) node[above]{$P_X(x)$};
					\foreach \x in {1, 2}
